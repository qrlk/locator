...
async def handle(request):
    ...
    if info:
        ...
        print(f"[{ip}]: Пришел запрос от {sender}. Он передал информацию о {len(info['vehicles'])} т/с и запросил координаты транспорта с model {request_model}. allow_unlocked: {allow_unlocked}, allow_occupied: {allow_occupied}")
        if ip not in data:
            data[ip] = {}
            data[ip]["vehicles"] = {}
            data[ip]["senders"] = {}
        if time.time() - last_clear > 30:
            print(f"[{ip}]: Запускаю очистку")
            del_list = []
            last_clear = time.time()
            for k, v in data[ip]["vehicles"].items():
                if int(time.time()) - v["timestamp"] > 25:
                    print(f"[{ip}]: Vehicle {k} теперь в списке смерти")
                    del_list.append(k)
            for item in del_list:
                del data[ip]["vehicles"][item]
                print(f"[{ip}]: Транспорт с ID {item} удален")
            del_list = []
            for k, v in data[ip]["senders"].items():
                print(k, v)
                if int(time.time()) - v > 25:
                    print(f"[{ip}]: Sender {k} добавлен в список удаления")
                    del_list.append(k)
            for item in del_list:
                del data[ip]["senders"][item]
                print(f"[{ip}]: Sender {item} удален")
        data[ip]["senders"][sender] = time.time()
        for item in info["vehicles"]:
            item["timestamp"] = time.time()
            data[ip]["vehicles"][item["id"]] = item
        print(
            f"""[{ip}]: Активных сендеров: {len(data[ip]["senders"])}, машин: {len(data[ip]["vehicles"])}""")
        if request_model != -1:
            response_cars = []
            ...
            if response_cars == []:
                print(f"[{ip}]: Подготовлен ответ для {sender}. Он хотел model {request_model}, но их не нашлось. (ОТКР: {allow_unlocked}, ЗАН: {allow_occupied}). Обработка заняла: {time.time()-timer} с")
                ...
                print(f"[{ip}]: Подготовлен ответ для {sender}. Он хотел model {request_model}, мы нашли {len(response_cars)} вариантов. (ОТКР: {allow_unlocked}, ЗАН: {allow_occupied}). Обработка заняла: {time.time()-timer} с")
            ...
            print(
                f"[{ip}]: Подготовлен ответ для {sender}. Он не запрашивал машину. Обработка заняла: {time.time()-timer} с")
